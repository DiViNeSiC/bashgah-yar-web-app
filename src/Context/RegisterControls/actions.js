...
export const registerUserSuccessDisable = (dispatch) => { dispatch({ type: REGISTER_SUCCESS_DISABLE }) }
...
export const registerNewUser = (registerType, formData) => async (dispatch, globalDispatch, history = null) => {
    try {
        enableLoading(globalDispatch)
        const url = registerType && `/registers/${registerType}`
        if (!url) return toasterError('متد رجیستر نباید خالی باشد')(globalDispatch)
...
export const sendActivationAccountEmail = async (dispatch, globalDispatch, history = null) => {
    try {
        enableLoading(globalDispatch)
        const response = await axios(history).post('/auth/active-account/email')
...
export const activeAccount = (accountActivationToken) => async (dispatch, globalDispatch, history = null) => {
    try {
        enableLoading(globalDispatch)
        const response = await axios(history).put(`/auth/active-account/${accountActivationToken}`)
